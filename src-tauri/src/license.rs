// Offline license validation: key = 6 groups of 4 [A-Z0-9], seed = first 5 groups
// joined, signature = group 6 = first 4 hex chars (uppercase) of SHA256(seed + salt).
// The salt must match the separate key-generator app (SingleUseApps-Portal).
...
mod tests {
    ...
    fn accepts_a_correctly_signed_key() {
        ...
        assert!(validate("A1B2-C3D4-E5F6-G7H8-J9K0-305B"));
        assert!(validate("a1b2-c3d4-e5f6-g7h8-j9k0-305b")); // case-insensitive
    }
