...
FileLister’s OneDrive stack. DupSweep site must **not** claim this until it ships. Suggested order (same as FileLister): **Files → Folders → Photos**.
...
1. **Files Ignore + auto-select keep rules** — everyday cleanup, no cloud.
2. **Photos size filter + auto-scan** — FileLister parity, small.
3. **Screenshots** on the site (and Help) when you have them.
4. **Stripe live** when you want paying customers.
5. **Cloud login** — OneDrive Files → Folders → Photos → multi-connection.
6. Everything else (Google Drive, dashboard, Finder extension, …) after that.
